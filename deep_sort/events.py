...
class PolygonZone:
    """A named polygon in image coordinates."""

    def __init__(self, name, polygon, color=(0, 0, 255)):
    ...
    def draw(self, frame, alpha=0.18):
        ...
        cv2.polylines(frame, [self.polygon], True, self.color, 2, cv2.LINE_AA)
        anchor = tuple(int(value) for value in self.polygon[0, 0])
        cv2.putText(
...
class EventDetector:
    ...
    def update(self, outputs, frame_idx, fps=15.0):
        ...
        for output in outputs:
            ...
            state["history"].append((current_frame, center))
            state["last_frame"] = current_frame
            state["class_id"] = class_id

            inside_zones = {
                zone.name for zone in self.zones if zone.contains(center)
            }
            ...
            for zone_name in sorted(inside_zones - previously_inside):
                events.append(self._event(
                    ...
                    current_frame, current_frame / fps, zone_name, center,
                ))
            ...
            if not self.loitering_enabled or not eligible_for_loitering:
                ...
                if (
                    ...
                    events.append(self._event(
                        ...
                        current_frame / fps, zone_name, center,
                    ))
            ...
            statuses[track_id] = {
                ...
                "history": [point for _, point in state["history"]],
            }
...
def load_event_config(config):
    ...
    for index, zone_cfg in enumerate(zone_configs):
        ...
        zones.append(PolygonZone(name, polygon, color))
    ...
    return EventDetector(
        ...
        loitering_max_movement=_value(loiter_cfg, "MAX_MOVEMENT_PIXELS", 60.0),
